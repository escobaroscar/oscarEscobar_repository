  ...
  private readonly axios:AxiosInstance= axios
    ...
    async create(createTribeDto: CreateTribeDto) {
      ...
      async findTribu(id: number) {
        ...
        let findTribu = await this.tribeRepository.findOne({  relations: ['organization','repositories.metric'], where: { id: id } });
       if (!findTribu)  throw new HttpException(`La tribu ${id} no se encuentra registrada`, HttpStatus.NOT_FOUND);
       if([0].includes(findTribu?.repositories.length||0)) throw new HttpException(`La tribu ${id} no posee repositorios agregados`, HttpStatus.ACCEPTED);
       
       findTribu?.repositories.map((repository, i) => { 
         if(repository?.metric && repository?.create_time.getFullYear()==currentYear && repository.metric.coverage > 75 &&  repository.state =='E'){
       ...
       if([0].includes(listRepositories.length||0)) throw new HttpException(`La tribu ${id} no tiene repositorios que cumplan  con la cobertura necesaria`, HttpStatus.ACCEPTED);
       const {data} = await this.axios.get(`${this.envConfig.get('API_URL')}/repository`);
       listRepositories.map((repository, i) => { 
       listRepositories[i]={...repository, verficactionState:this.searchState(repository?.id,data)};
       });
